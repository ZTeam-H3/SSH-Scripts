...
func (conn *Connection) SendCommands(cmds []string,delay int) ([]byte, error) {
...
func Sshconfig(username,auth string)(config *ssh.ClientConfig){
	...
	if strings.HasPrefix(auth, "pk:") {
		config.Auth = []ssh.AuthMethod{
			publicKeyAuthFunc(auth[3:]),
		}
	} else {
		config.Auth = []ssh.AuthMethod{
			ssh.Password(auth),
